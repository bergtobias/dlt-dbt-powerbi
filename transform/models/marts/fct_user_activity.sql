-- Cross-analysis: user engagement across posts, todos, and cart spend
with users as (
    select * from {{ source('dummyjson', 'users') }}
),
...
todos as (
    ...
    from {{ source('dummyjson', 'dj_todos') }}
    group by user_id
),
